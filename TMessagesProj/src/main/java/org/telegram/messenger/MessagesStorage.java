    ...
    public void openDatabase(int openTries) {
        ...
        try {
            ...
            if (createTable) {
                ...
                // kg begin - migrations
                if (!database.tableExists("kg_params")) {
                    database.executeFast("CREATE TABLE IF NOT EXISTS kg_params(version INTEGER)").stepThis().dispose();
                    database.executeFast("INSERT INTO kg_params VALUES(0)").stepThis().dispose();
                }
                int kg_version = database.executeInt("SELECT version FROM kg_params LIMIT 1");
                // see updateDbToLastVersion, DatabaseMigrationHelper.migrate
                if (kg_version == 0) {
                    kg_version = 1;
                    database.executeFast("ALTER TABLE dialogs ADD COLUMN top_message_nb INTEGER default 0").stepThis().dispose();
                    database.executeFast("ALTER TABLE dialogs ADD COLUMN last_message_date_nb INTEGER default 0").stepThis().dispose();
                    database.executeFast(String.format(Locale.US, "UPDATE kg_params SET version = %d", kg_version)).stepThis().dispose();
                }
                if (kg_version == 1) {
                    kg_version = 2;
                    database.executeFast("ALTER TABLE topics ADD COLUMN top_message_nb INTEGER default 0").stepThis().dispose();
                    database.executeFast(String.format(Locale.US, "UPDATE kg_params SET version = %d", kg_version)).stepThis().dispose();
                }
//                if (kg_version == 2) {
//                    kg_version = 3;
...
//                }
    ...
    public static void createTables(SQLiteDatabase database) throws SQLiteException {
        ...
        database.executeFast("CREATE TABLE dialogs(did INTEGER PRIMARY KEY, date INTEGER, unread_count INTEGER, last_mid INTEGER, inbox_max INTEGER, outbox_max INTEGER, last_mid_i INTEGER, unread_count_i INTEGER, pts INTEGER, date_i INTEGER, pinned INTEGER, flags INTEGER, folder_id INTEGER, data BLOB, unread_reactions INTEGER, last_mid_group INTEGER, ttl_period INTEGER, top_message_nb INTEGER, last_message_date_nb INTEGER)").stepThis().dispose();
        // kg end
        ...
        database.executeFast("CREATE TABLE topics(did INTEGER, topic_id INTEGER, data BLOB, top_message INTEGER, topic_message BLOB, unread_count INTEGER, max_read_id INTEGER, unread_mentions INTEGER, unread_reactions INTEGER, read_outbox INTEGER, pinned INTEGER, total_messages_count INTEGER, hidden INTEGER, edit_date INTEGER, top_message_nb INTEGER, PRIMARY KEY(did, topic_id));").stepThis().dispose();
        // kg end
        ...
        // kg begin
        // database.executeFast("CREATE TABLE blocked_users(uid INTEGER PRIMARY KEY);").stepThis().dispose(); // kg
        database.executeFast("CREATE TABLE IF NOT EXISTS kg_params(version INTEGER)").stepThis().dispose();
        int lastKgVersion = 2;
        database.executeFast(String.format(Locale.US, "INSERT INTO kg_params VALUES(%d)", lastKgVersion)).stepThis().dispose();
        // kg end
    ...
    private void saveTopicsInternal(long dialogId, List<TLRPC.TL_forumTopic> topics, boolean replace, boolean inTransaction, int date) {
        ...
        try {
            ...
            for (int i = 0; i < topics.size(); i++) {
                ...
                state.bindInteger(15, topic.topMessageNB != null ? topic.topMessageNB.id : 0); // kg
    ...
    public void loadTopics(long dialogId, Consumer<ArrayList<TLRPC.TL_forumTopic>> callback) {
        storageQueue.postRunnable(() -> {
            ...
            try {
                ...
                cursor = database.queryFinalized(String.format(Locale.US, "SELECT top_message, data, topic_message, unread_count, max_read_id, unread_mentions, unread_reactions, read_outbox, pinned, total_messages_count, top_message_nb FROM topics WHERE did = %d ORDER BY pinned ASC", dialogId));

                SparseArray<ArrayList<TLRPC.TL_forumTopic>> topicsByTopMessageId = null;
                HashSet<Integer> topMessageIds = null;
                HashSet<Integer> topMessageIdsSameNB = null; // kg
                HashSet<Integer> topMessageIdsDifferentNB = null; // kg
                while (cursor.next()) {
                    if (topics == null) {
                        topics = new ArrayList<>();
                        topicsByTopMessageId = new SparseArray<>();
                        topMessageIds = new HashSet<>();
                        topMessageIdsSameNB = new HashSet<>(); // kg
                        topMessageIdsDifferentNB = new HashSet<>(); // kg
                    }
                    int topMessageId = cursor.intValue(0);
                    int topMessageIdNB = cursor.intValue(10); // kg
                    NativeByteBuffer data = cursor.byteBufferValue(1);
                    if (data != null) {
                        ...
                        if (topic != null) {
                            ...
                            // kg begin
                            if (topMessageIdNB != 0) {
                                if (topMessageIdNB == topMessageId) {
                                    topMessageIdsSameNB.add(topMessageIdNB);
                                } else {
                                    topMessageIdsDifferentNB.add(topMessageIdNB);
                                }
                            }
                            // kg end
                            topics.add(topic);
                ...
                if (topics != null && !topics.isEmpty()) {
                    ...
                    // kg begin - add topMessageNB to all the topics! Copied from the same code above - see it

                    // 1. process topics with topMessageIdNB == topMessageId
                    for (int messageId : topMessageIdsSameNB) {
                        ArrayList<TLRPC.TL_forumTopic> topicsList = topicsByTopMessageId.get(messageId);
                        if (topicsList != null) {
                            for (int i = 0; i < topicsList.size(); i++) {
                                topicsList.get(i).topMessageNB = topicsList.get(i).topMessage;
                            }
                        }
                    }

                    // 2. add different topMessageIdNB from messages_v2
                    cursor2.dispose();
                    if (!topMessageIdsDifferentNB.isEmpty()) {
                        cursor2 = database.queryFinalized("SELECT mid, data, replydata FROM messages_v2 WHERE uid = " + dialogId + " AND mid IN (" + TextUtils.join(",", topMessageIdsDifferentNB) + ")");
                        while (cursor2.next()) {
                            int messageId = cursor2.intValue(0);
                            NativeByteBuffer data = cursor2.byteBufferValue(1);
                            if (data != null) {
                                ...
                                topMessageIdsDifferentNB.remove(messageId);
                                addUsersAndChatsFromMessage(message, usersToLoad, chatsToLoad, null);

                                ArrayList<TLRPC.TL_forumTopic> topicsList = topicsByTopMessageId.get(messageId);
                                if (topicsList != null) {
                                    for (int i = 0; i < topicsList.size(); i++) {
                                        topicsList.get(i).topMessageNB = message;
                                    }
                                }
...
//                            }
                    ...
                    // 3. add different topMessageIdNB from messages_topics
                    cursor2.dispose();
                    if (!topMessageIdsDifferentNB.isEmpty()) {
                        cursor2 = database.queryFinalized("SELECT mid, data FROM messages_topics WHERE uid = " + dialogId + " AND mid IN (" + TextUtils.join(",", topMessageIdsDifferentNB) + ")");
                        try {
                            while (cursor2.next()) {
                                int messageId = cursor2.intValue(0);
                                NativeByteBuffer data = cursor2.byteBufferValue(1);
    ...
    // kg begin
    private void resetDialogNbFieldsByRealLastMessage(TLRPC.Dialog dialog, TLRPC.Message message) {
        if (dialog.last_message_date_nb > dialog.last_message_date) {
            if (getMessagesController().userInBlockedSet(message)) {
                dialog.last_message_date_nb = 0;
                dialog.top_message_nb = 0;
            } else {
                dialog.last_message_date_nb = dialog.last_message_date;
                dialog.top_message_nb = message.id; // dialog.top_message ???
            }
        }
    }
    // kg end
    ...
    private TLRPC.messages_Dialogs loadDialogsByIds(String ids, ArrayList<Long> usersToLoad, ArrayList<Long> chatsToLoad, ArrayList<Integer> encryptedToLoad) throws Exception {
        ...
        try {
            ...
            cursor = database.queryFinalized(String.format(Locale.US, "SELECT d.did, d.last_mid, d.unread_count, d.date, m.data, m.read_state, m.mid, m.send_state, s.flags, m.date, d.pts, d.inbox_max, d.outbox_max, m.replydata, d.pinned, d.unread_count_i, d.flags, d.folder_id, d.data, d.unread_reactions, d.last_mid_group, d.ttl_period, d.top_message_nb, d.last_message_date_nb FROM dialogs as d LEFT JOIN messages_v2 as m ON d.last_mid = m.mid AND d.did = m.uid LEFT JOIN dialog_settings as s ON d.did = s.did WHERE d.did IN (%s) ORDER BY d.pinned DESC, d.date DESC", ids));
            while (cursor.next()) {
                ...
                // kg
                dialog.top_message_nb = cursor.intValue(22);
                dialog.last_message_date_nb = cursor.intValue(23);
                ...
                if (data != null) {
                    ...
                    if (message != null) {
                        ...
                        if (date != 0) {
                            ...
                            resetDialogNbFieldsByRealLastMessage(dialog, message); // kg
                        }
...
//                }
        ...
        storageQueue.postRunnable(() -> {
            ...
            try {
                ...
                if (dids.isEmpty()) {
                    ...
                    cursor = database.queryFinalized("SELECT d.did, d.last_mid, d.unread_count, d.date, m.data, m.read_state, m.mid, m.send_state, m.date, d.top_message_nb, d.last_message_date_nb FROM dialogs as d LEFT JOIN messages_v2 as m ON d.last_mid = m.mid AND d.did = m.uid WHERE d.folder_id = 0 ORDER BY d.pinned DESC, d.date DESC LIMIT 0,10");
                } else {
                    // kg
                    ...
                    cursor = database.queryFinalized(String.format(Locale.US, "SELECT d.did, d.last_mid, d.unread_count, d.date, m.data, m.read_state, m.mid, m.send_state, m.date, d.top_message_nb, d.last_message_date_nb FROM dialogs as d LEFT JOIN messages_v2 as m ON d.last_mid = m.mid AND d.did = m.uid WHERE d.did IN(%s)", TextUtils.join(",", dids)));
                }
                while (cursor.next()) {
                    long dialogId = cursor.longValue(0);
                    if (DialogObject.isFolderDialogId(dialogId)) {
                    ...
                    dialog.last_message_date = cursor.intValue(3);
                    // kg
                    dialog.top_message_nb = cursor.intValue(9);
                    dialog.last_message_date_nb = cursor.intValue(10);
        ...
        storageQueue.postRunnable(() -> {
            ...
            try {
                ...
                if (dialog != null) {
                    ...
                    // kg
                    state.bindInteger(18, dialog.top_message_nb);
                    state.bindInteger(19, dialog.last_message_date_nb);
    ...
    // kg begin
    private static int getMessageIdOrLocalIdOrDefault(TLRPC.Message message, int defaultValue) {
        int messageId = message != null ? message.id : defaultValue;
        if (message != null) {
...
//                        messageId = message.local_id;
//                    }
                    int messageId = getMessageIdOrLocalIdOrDefault(message, 0);
                    // kg end
...
//                state_randoms = null;
                ...
                LongSparseArray<TLRPC.Message> messagesMapNB = new LongSparseArray<>(); // kg
                ...
                HashMap<TopicKey, TLRPC.Message> topicMessagesMapNB = new HashMap<>(); // kg
...
//                        cursor = null;
                    ...
                    if (updateDialog) {
                        ...
                        // kg begin
                        if (!getMessagesController().userInBlockedSet(message)) {
                            lastMessage = messagesMapNB.get(message.dialog_id);
                            if (lastMessage == null || message.date > lastMessage.date || lastMessage.id > 0 && message.id > lastMessage.id || lastMessage.id < 0 && message.id < lastMessage.id) {
                                messagesMapNB.put(message.dialog_id, message);
                            }
                            if (topicId != 0 && message.dialog_id != selfId) {
                                TopicKey topicKey = TopicKey.of(message.dialog_id, topicId);
                                lastMessage = topicMessagesMapNB.get(topicKey);
                                if (lastMessage == null || message.date > lastMessage.date || lastMessage.id > 0 && message.id > lastMessage.id || lastMessage.id < 0 && message.id < lastMessage.id) {
                                    topicMessagesMapNB.put(topicKey, message);
                                }
                            }
                        }
                        // kg end
                ...
                state_dialogs_update = database.executeFast("UPDATE dialogs SET date = ?, unread_count = ?, last_mid = ?, last_mid_group = ?, unread_count_i = ?, top_message_nb = ?, last_message_date_nb = ? WHERE did = ?");
                ...
                state_topics_update = database.executeFast("UPDATE topics SET unread_count = ?, top_message = ?, unread_mentions = ?, total_messages_count = ?, top_message_nb = ? WHERE did = ? AND topic_id = ?");
                ...
                for (int a = 0; a < messagesMap.size(); a++) {
                    ...
                    TLRPC.Message messageNB = messagesMapNB.get(key); // kg
                    ...
                    cursor = database.queryFinalized("SELECT date, unread_count, last_mid, unread_count_i, top_message_nb, last_message_date_nb FROM dialogs WHERE did = " + key);
                    int dialog_date = 0;
                    int last_mid = 0;
                    int old_unread_count = 0;
                    int old_mentions_count = 0;
                    // kg
                    int top_message_nb = 0;
                    int last_message_date_nb = 0;
                    boolean exists;
                    if (exists = cursor.next()) {
                        ...
                        // kg
                        top_message_nb = Math.max(0, cursor.intValue(4));
                        last_message_date_nb = Math.max(0, cursor.intValue(5));
                    } else if (channelId != 0) {
                        getMessagesController().checkChatInviter(channelId, true);
                    }
                    cursor.dispose();
...
//                        }
//                    }
                    int messageId = getMessageIdOrLocalIdOrDefault(message, last_mid);
                    int messageIdNB = getMessageIdOrLocalIdOrDefault(messageNB, top_message_nb);
                    // kg end
                    ...
                    if (exists) {
                        if (message == null || (DialogObject.isEncryptedDialog(key) ? message.date > dialog_date : message.id >= last_mid) || message.send_state != 0 && (message.flags & TLRPC.MESSAGE_FLAG_EDITED) == 0) {
                            ...
                            state_dialogs_update.bindInteger(6, messageIdNB);
                            state_dialogs_update.bindInteger(7, messageNB != null && (!doNotUpdateDialogDate || last_message_date_nb == 0) ? messageNB.date : last_message_date_nb);
                            state_dialogs_update.bindLong(8, key);
                            // kg end
                        ...
                        // kg
                        state_dialogs_replace.bindInteger(18, messageIdNB);
                        state_dialogs_replace.bindInteger(19, messageNB != null && (!doNotUpdateDialogDate || last_message_date_nb == 0) ? messageNB.date : last_message_date_nb);
                ...
                while (iterator.hasNext()) {
                    ...
                    TLRPC.Message messageNB = topicMessagesMapNB.get(topicKey); // kg
                    ...
                    cursor = database.queryFinalized("SELECT unread_count, top_message, unread_mentions, total_messages_count, top_message_nb FROM topics WHERE did = " + topicKey.dialogId + " AND topic_id = " + topicKey.topicId);
                    int oldUnreadCount = 0;
                    int oldMentions = 0;
                    int oldTotalCount = 0;
                    int topMessage = 0;
                    int topMessageNB = 0; // kg
                    int newMentions = 0;
                    ...
                    if (cursor.next()) {
                        ...
                        topMessageNB = cursor.intValue(4); // kg
                    }
                    cursor.dispose();
                    cursor = null;
                    if (!exist) {
...
//                        }
//                    }
                    int messageId = getMessageIdOrLocalIdOrDefault(message, topMessage);
                    int messageIdNB = getMessageIdOrLocalIdOrDefault(messageNB, topMessageNB);
                    // kg end
                    ...
                    state_topics_update.bindInteger(5, messageIdNB);
                    state_topics_update.bindLong(6, topicKey.dialogId);
                    state_topics_update.bindLong(7, topicKey.topicId);
                    // kg end
                    ...
                    if (message != null) {
                        ...
                        topicUpdate.topMessageNB = messageNB; // kg
    ...
    private long[] updateMessageStateAndIdInternal(long randomId, long dialogId, Integer _oldId, int newId, int date, int scheduled, int newTopicId) {
        ...
        if (oldMessageId == newId && date != 0) {
            ...
            if (scheduled == 0) {
                ...
                try {
                    state = database.executeFast("UPDATE dialogs SET last_mid = ? WHERE last_mid = ?"); // kg here?
                    state.bindInteger(1, newId);
                    state.bindInteger(2, oldMessageId);
                    state.step();
                } catch (Exception e) {
...
//                }
    ...
    private void updateDialogsWithDeletedMessagesInternal(long originalDialogId, long channelId, ArrayList<Integer> messages, ArrayList<Long> additionalDialogsToUpdate) {
        ...
        try {
            ...
            cursor = database.queryFinalized(String.format(Locale.US, "SELECT d.did, d.last_mid, d.unread_count, d.date, m.data, m.read_state, m.mid, m.send_state, m.date, d.pts, d.inbox_max, d.outbox_max, d.pinned, d.unread_count_i, d.flags, d.folder_id, d.data, d.unread_reactions, d.last_mid_group, d.ttl_period, d.top_message_nb, d.last_message_date_nb FROM dialogs as d LEFT JOIN messages_v2 as m ON d.last_mid = m.mid AND d.did = m.uid AND d.last_mid_group IS NULL WHERE d.did IN(%s)", ids));
            while (cursor.next()) {
                long dialogId = cursor.longValue(0);
                TLRPC.Dialog dialog;
                if (DialogObject.isFolderDialogId(dialogId)) {
                ...
                // kg
                dialog.top_message_nb = cursor.intValue(20);
                dialog.last_message_date_nb = cursor.intValue(21);
...
//                    broadcastScheduledMessagesChange(dialogId);
                    ...
                    // kg begin
                    // relying on sorted messages by date!!!
                    TLRPC.Message messageNB = null;
                    for (int a = 0; a < count; a++) {
                        TLRPC.Message message = messages.messages.get(a);
                        if (!getMessagesController().userInBlockedSet(message)) {
                            messageNB = message;
                    ...
                    for (int a = 0; a < count; a++) {
                        ...
                        if (a == 0 && createDialog) { // kg - relying on sorted messages by date??
                            int pinned = 0;
                            int mentions = 0;
                            int flags = 0;
                            ...
                            if (exist) {
                                ...
                                if (messageNB != null) {
                                    state3 = database.executeFast("UPDATE dialogs SET date = ?, last_mid = ?, last_mid_group = ?, inbox_max = ?, last_mid_i = ?, pts = ?, date_i = ?, top_message_nb = ?, last_message_date_nb = ? WHERE did = ?");
                                } else {
                                    state3 = database.executeFast("UPDATE dialogs SET date = ?, last_mid = ?, last_mid_group = ?, inbox_max = ?, last_mid_i = ?, pts = ?, date_i = ? WHERE did = ?");
                                }
                                ...
                                if (messageNB != null) {
                                    state3.bindInteger(8, messageNB.id);
                                    state3.bindInteger(9, messageNB.date);
                                    state3.bindLong(10, dialogId);
                                } else {
                                    state3.bindLong(8, dialogId);
                                }
                                // kg end
                                ...
                                // kg
                                state3.bindInteger(18, messageNB != null ? messageNB.id : 0);
                                state3.bindInteger(19, messageNB != null ? messageNB.date : 0);
        ...
        try {
            ...
            if (!dialogs.dialogs.isEmpty()) {
                ...
                for (int a = 0; a < dialogs.dialogs.size(); a++) {
                    ...
                    // kg
                    state_dialogs.bindInteger(18, dialog.top_message_nb);
                    state_dialogs.bindInteger(19, dialog.last_message_date_nb);
...
//            dialogs.messages.add(message);
    ...
    private LongSparseArray<MessageObject> lastMessagesNBCache = new LongSparseArray<>();

    public MessageObject getLastMessageFromNonBlockedUser(TLRPC.Dialog dialog) {

        int messageId = dialog.top_message_nb;
        if (messageId == 0) {
            return null;
        }
        ...
        // check cached value
        MessageObject messageObject = lastMessagesNBCache.get(dialogId);
        if (messageObject != null && messageObject.getId() == messageId) {
            // System.out.println("" + dialogId + " " + messageId + ": FOUND IN STORAGE CACHE!");
            return messageObject;
        }
        // System.out.println("" + dialogId + " " + messageId + ": MAKING STORAGE QUERY");
        ...
        try {
            boolean read = false;
            NativeByteBuffer data = null;
            // cursor = database.queryFinalized("SELECT data, mid FROM messages_v2 WHERE uid = ? ORDER BY mid DESC LIMIT 1 OFFSET 1", dialogId);
            ...
            if (!read) {
                cursor.dispose();
                // cursor = database.queryFinalized("SELECT data, mid FROM messages_topics WHERE uid = ? ORDER BY mid DESC LIMIT 1 OFFSET 1", dialogId);
                cursor = database.queryFinalized("SELECT data FROM messages_topics WHERE mid = ? AND uid = ?", messageId, dialogId);
                if (cursor.next()) {
                    read = true;
                    data = cursor.byteBufferValue(0);
                }
...
//              message.dialog_id = -channel_id;
        ...
        if (message != null) {
            messageObject = new MessageObject(currentAccount, message, true, true); // true, true) ???
            lastMessagesNBCache.put(dialogId, messageObject);

            // just for loading non blocked user if it is not loaded yet - see loadTopics
            try {
                ArrayList<Long> usersToLoad = new ArrayList<>();
                ArrayList<Long> chatsToLoad = new ArrayList<>();

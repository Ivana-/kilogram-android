...
public class TopicsController extends BaseController {
        ...
        getConnectionsManager().sendRequest(getForumTopics, (response, error) -> {
            if (response != null) {
                ...
                for (int i = 0; i < topics.messages.size(); i++) {
                    messagesMap.put(topics.messages.get(i).id, topics.messages.get(i)); // kg here
                }
                AndroidUtilities.runOnUIThread(() -> {
            ...
            for (int i = 0; i < newTopics.size(); i++) {
                ...
                if (!topicsMap.containsKey(newTopic.id)) {
                    if (messagesMap != null) {
                        ...
                        newTopic.topMessageNB = null; // kg here add real NB!
    ...
    // kg begin
    private int topicTopMessageDate(TLRPC.TL_forumTopic topic) {
        if (topic == null || topic.topMessage == null) {
            return 0;
        }
        TLRPC.Message message = topic.topMessage;
        if (getMessagesController().kgModeAndUserInBlockedSet(topic.topMessage) && topic.topMessageNB != null) {
            message = topic.topMessageNB;
        }
        return message.date;
    }
    // kg end
        ...
        if (topics != null) {
            if (openedTopicsBuChatId.get(chatId, 0) > 0) {
                Collections.sort(topics, (a, b) -> {
                    ...
                    return topicTopMessageDate(b) - topicTopMessageDate(a);
                });
            }
            if (notify) {
                getNotificationCenter().postNotificationName(NotificationCenter.topicsDidLoaded, chatId, true);
                        ...
                        for (int i = 0; i < topicsToUpdate.size(); i++) {
                            ...
                            if (cursor.next()) {
                                ...
                                if (data != null) {
                                    ...
                                    topic.topMessageNB = null; // kg here add real NB!
                        ...
                        for (int i = 0; i < finalTopicsToUpdate.size(); i++) {
                            ...
                            if (map != null) {
                                ...
                                if (toUpdate != null && fromUpdate.top_message != -1 && fromUpdate.topMessage != null) {
                                    ...
                                    toUpdate.topMessageNB = fromUpdate.topMessageNB; // kg
                ...
                if (update.reloadTopic) {
                    ...
                    if (topic != null) {
                        if (update.onlyCounters) {
                            ...
                            topic.topMessageNB = update.topMessageNB; // kg

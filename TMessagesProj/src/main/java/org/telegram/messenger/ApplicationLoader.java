...
public class ApplicationLoader extends Application {
    ...
    // kg begin
    private static long kgFirstStartTime;
    public static volatile boolean kgActivated = false;
    // kg end
    ...
    public void onCreate() {
        ...
        // kg begin
        SharedPreferences preferences = applicationContext.getSharedPreferences("kg", Activity.MODE_PRIVATE);
        kgFirstStartTime = preferences.getLong("firstStartTime", 0);
        if (kgFirstStartTime == 0) {
            kgFirstStartTime = System.currentTimeMillis();
            preferences.edit().putLong("firstStartTime", kgFirstStartTime).commit(); // apply();
        }
        String kgKey = preferences.getString("activationKey", null);
        if (kgKey != null) {
            kgActivated = true;
        }
        // kg end
    ...
    // kg begin
    public static String generateKgActivationKey(long userId) {
        if (userId == 0) {
            return null;
        }
        String str = "֗" + BuildVars.APP_ID + userId + BuildVars.APP_HASH;
        return Utilities.bytesToHex(Utilities.computeSHA256(Util.getUtf8Bytes(str))).substring(0, 10).toLowerCase();
    }

    public static boolean checkKgActivationKey(long userId, String kgKey) {
        if (userId == 0 || kgKey == null || kgKey.isEmpty()) {
            return false;
        }
        boolean ok = kgKey.equals(generateKgActivationKey(userId));
        if (ok) {
            SharedPreferences preferences = applicationContext.getSharedPreferences("kg", Activity.MODE_PRIVATE);
            preferences.edit().putString("activationKey", kgKey).commit(); // apply();
            kgActivated = true;
            NotificationCenter.getGlobalInstance().postNotificationName(NotificationCenter.kgModeToggled); // for ui change
        }
        return ok;
    }

    public static long kgFreeTrialRemainingTime() {
        long freeTrialDays = 3; // 3 free days from kgFirstStartTime before activation is required
        return ApplicationLoader.kgFirstStartTime + freeTrialDays * 24 * 60 * 60 * 1000 - System.currentTimeMillis();
    }

    public static boolean kgModeAllowed() {
        return ApplicationLoader.kgActivated || ApplicationLoader.kgFreeTrialRemainingTime() > 0;
    }
    // kg end
